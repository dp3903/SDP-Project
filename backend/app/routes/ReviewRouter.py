...
async def create_review(review : ReviewModel):
	...
	new_review = await fb.reviews.find_one({"_id":result.inserted_id})
	return new_review

@reviewRouter.get("/{userId}",response_model=List[ReviewModel])
...
@reviewRouter.get("/{resourceId}",response_model=List[ReviewModel])
